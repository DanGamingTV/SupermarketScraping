...
config = {'siteMeta': {'name': 'Mighty Ape', 'mainURL': 'www.mightyape.co.nz', 'productPrefix': ''}, 'regex': {
    'dollars': '(\d{1,})', 'cents': '(\d{2,})', 'multipackDetect': '(\d{1,}ml).{0,}\((\d{1,}).{0,}p.{0,}k\)'}, 'endpoints': {}}
...
async def getProductPrice(productId):
    ...
    with requests.session() as s:
        ...
        productImageURL = str(productImageURL[0].find(
            'img')['src']) if len(productImageURL) > 0 else ''
        productFormat = soup.find_all('div', {'class': 'static'})
        productFormat = productFormat[0] if len(productFormat) > 0 else ''
        productFormat = productFormat.find('span').text.strip() if len(
            productFormat.find_all('span')) > 0 else ''
        productFormatMultipackDetect = re.findall(
            "Pack of (\d{1,})", productFormat)
        centsprice = re.findall(config['regex']['cents'], scrapedData['cents'])
        dollarsprice = re.findall(config['regex']['dollars'], dollars)
        multipack_detect = re.findall(
            config['regex']['multipackDetect'], productName.lower())
        ...
        if (productFormatMultipackDetect):
            price['productData']['multipack'] = {
                'quantity': productFormatMultipackDetect[0]}
        if (multipack_detect):
            if ('multipack' not in price['productData']):
                price['productData']['multipack'] = {
                    'quantity': list(multipack_detect[0])[1]}
