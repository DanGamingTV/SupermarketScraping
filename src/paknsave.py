...
config = {'siteMeta': {'name': 'Paknsave', 'mainURL': 'https://www.paknsave.co.nz', 'productPrefix': 'pns'},
          'regex': {'dollars': '>([0-9][0-9]?)', 'cents': '>([0-9][0-9])', 'volumeData': "Serving.pack: (\d{1,}) Serving size: (.{1,})"}}
...
def getStoreIDs():
    ...
    with requests.session() as s:
        r = s.get(
            f"{config['siteMeta']['mainURL']}/CommonApi/Store/GetStoreList")
...
async def getProductPrice(productId, storeId):
    productId = productId.replace(config['siteMeta']['productPrefix'], '') if productId.endswith(
        config['siteMeta']['productPrefix']) else productId  # If productId has a prefix added to it, remove it.
    # URL to change store
    url = f"{config['siteMeta']['mainURL']}/CommonApi/Store/ChangeStore?storeId={storeId}&clickSource=list"
    ...
    with requests.session() as s:
        ...
        scrapedData['cents'] = str(soup.find_all(
            'span', {'class': "fs-price-lockup__cents"}))
        scrapedData['dollars'] = str(soup.find_all(
            'span', {'class': "fs-price-lockup__dollars"}))
        scrapedData['productName'] = soup.find_all(
            'h1', {'class': "u-h4 u-color-dark-grey"})
        if (len(scrapedData['productName']) > 0):
            scrapedData['productName'] = str(
                scrapedData['productName'][0].contents[0])
        scrapedData['productImageURL'] = "https://a.fsimg.co.nz/product/retail/fan/image/master/" + \
            productId.replace("_ea_000", "") + ".png"
        scrapedData['productDescription'] = soup.find_all(
        ...
        scrapedData['volumeText'] = soup.find_all(
            'div', {'class': 'fs-accordion__accesible-panel'})
        if (len(scrapedData['volumeText']) > 0):
            scrapedData['volumeText'] = scrapedData['volumeText'][1].find(
                'p').text
            scrapedData['volumeData'] = list(re.findall(
                config['regex']['volumeData'], scrapedData['volumeText'])[0])
        else:
            scrapedData['volumeData'] = ['', '']
        scrapedData['ppl'] = soup.find_all(
            'div', {'class': 'fs-product-card__price-by-weight'})
        scrapedData['pplValid'] = False
        if (len(scrapedData['ppl']) > 0):
            scrapedData['ppl'] = str(
                scrapedData['ppl'][0].contents[0]).replace('$', '')
            scrapedData['pplValid'] = True
            if (scrapedData['ppl'].endswith(' per 100ml')):
                scrapedData['ppl'] = scrapedData['ppl'].replace(
                    ' per 100ml', '')
        ...
        dollarsprice = re.findall(
            config['regex']['dollars'], scrapedData['dollars'])
        ...
        price = {'productData': {'name': scrapedData['productName'], 'productId': productId, 'productShopPage': config['siteMeta']['mainURL']+'/shop/product/'+productId+config['siteMeta']['productPrefix'], 'productImageURL': scrapedData['productImageURL'],
                                 'productDescription': scrapedData['productDescription'], 'volume': scrapedData['volumeData'][1], 'multipack': {'quantity': scrapedData['volumeData'][0]}}, 'bestPrice': salePrice, 'price': salePrice}
        ...
        if (scrapedData['pplValid'] == False):
            if ('volume' in price['productData']):
                if (len(price['productData']['volume'].lower().replace('ml', '')) > 0):
                    mutatedVolume = float(
                        price['productData']['volume'].lower().replace('ml', ''))
                    ...
                    calculatedPricePerLitre = float(
                        price['price'])*(1000/actualVolume)
        ...
        if (('pricePerLitre' in price) and ('multipack' in price['productData'])):
            if (price['pricePerLitre'] > 16 and price['productData']['multipack']['quantity'] != ''):
                ...
                price['pricePerLitre'] = price['pricePerLitre'] / \
                    int(price['productData']['multipack']['quantity'])
                price['bestPricePerLitre'] = price['bestPricePerLitre'] / \
                    int(price['productData']['multipack']['quantity'])
    try:
        mbR = s.get(config['siteMeta']['mainURL'] +
                    "/CommonApi/PromoGroup/GetPromoGroup?productId="+productId)
        try:
            ...
            if (mbData['success'] == True):
                ...
                if (multibuyQuantity > 1):
                    price['multiBuy'] = {
                        'quantity': multibuyQuantity, 'value': multibuyPrice, 'perUnit': multibuyPrice/multibuyQuantity}
